...
object QzDwsPaperDao {
  def getDwdQzPaperView(spark: SparkSession, dt: String) = {
  ...
  def getDwdQzCenterPaper(spark: SparkSession, dt: String) = {
    spark.sql(s"select paperviewid,sequence,centerid,dn from dwd.dwd_qz_center_paper where dt='$dt'")
  }

  def getDwdQzPaper(spark: SparkSession, dt: String) = {
  ...
  def getDwdQzCenter(spark: SparkSession, dt: String) = {
